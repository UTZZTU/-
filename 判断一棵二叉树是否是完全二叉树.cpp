...
class BinaryTree{             //二叉树类定义 
	...
	bool IsComplete(BinTreeNode<T>* root); //判断二叉树是否为完全二叉树 
...
template<class T>
bool BinaryTree<T>::IsComplete(BinTreeNode<T>* root)  //判断是否为完全二叉树 
{
    //1.树为空，返回错误
   if (root==NULL)
   {
...
int main ()
{
	...
	scanf("%s",u);
	int n=0;
	tree.createBinTree_pre(u,tree.root,n);   //建树 
	if(tree.IsComplete(tree.root))  //判断 
	cout<<"YES";
	else
	cout<<"NO";
	return 0;
