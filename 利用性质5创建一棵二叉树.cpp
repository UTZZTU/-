...
class BinaryTree{             //二叉树类定义 
	...
	void CreateBiTree( BinTreeNode<T> *& root, char data[], int pos, int upper );  //利用性质5建树 
...
void BinaryTree<T>::Traverse(BinTreeNode<T> *subTree,int k)
{
	...
	if(subTree!=NULL)
	{
		cout<<subTree->data<<endl;   //输出subTree的数据值 
...
template<class T>
void BinaryTree<T>::CreateBiTree( BinTreeNode<T> *&root, char data[], int pos, int upper ) //利用性质5建树 
{
	if (data[pos]=='#' || pos>upper )  root=NULL; 
    else
    {   
	    root=new BinTreeNode<T>(data[pos]);   //创建一个新结点
        CreateBiTree(root->leftChild, data, 2*pos, upper);  //递归左子树 
        CreateBiTree(root->rightChild, data, 2*pos+1, upper);  //递归右子树 
    }
}
int main ()
{
	...
	char u[100010];
	scanf("%s",u);
	tree.CreateBiTree(tree.root,u,1,strlen(u)-1);  //建树 
	tree.PreOrder();  //前序遍历 
	cout<<endl;
	tree.InOrder();  //中序遍历 
	return 0;
