...
void solve()
{
	...
	for(int i=1;i<=n;i++)
	{
		...
		cin>>p1>>p2>>s;
		Ld li={p1,p2};
		auto [u,v,w]=lineCircleCross(li,pcircle,r);
		if(u==0) continue;
		else if(u==1)
		{
			ld agl=angle(v-p1,p2-p1);
//			cout<<"agl "<<agl<<endl;
			if(!equal(0.0,agl)) continue;
			ld t=dis(v,p1)/s;
			pp[++cnt]={t,t+x};
//			cc(7)<<t<<endl;
		}
...
//			cout<<"agl "<<agl<<endl;
			if(!equal(0.0,agl)) continue;
			ld t1=dis(v,p1)/s,t2=dis(w,p1)/s;
			pp[++cnt]={min(t1,t2),max(t1,t2)+x};
//			cc(7)<<t1<<" "<<t2<<endl;
		}
	}
	sort(pp+1,pp+1+cnt);
	...
	{
		ld l=pp[1].x,r=pp[1].y;
		for(int i=2;i<=cnt;i++)
		{
			if(pp[i].x<=r) r=max(r,pp[i].y);
			else
			{
				ti+=r-l;
				l=pp[i].x,r=pp[i].y;
